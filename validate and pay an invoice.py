...
self.signal_workflow(cr, uid, [object.id], 'invoice_open')

context = dict(context or {})
context.update({
	'invoice_id': object.id,
	'invoice_type': object.type,
	'type': object.type in ('out_invoice','out_refund') and 'receipt' or 'payment',
	'payment_expected_currency': object.currency_id.id,
	'default_partner_id': self.pool['res.partner']._find_accounting_partner(object.partner_id).id,
	'default_amount': object.type in ('out_refund', 'in_refund') and -object.residual or object.residual,
	'default_reference': object.name,
	'default_type': object.type in ('out_invoice','out_refund') and 'receipt' or 'payment',
